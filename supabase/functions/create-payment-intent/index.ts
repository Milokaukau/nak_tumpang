  ...
  try {
    ...
    if (typeof request_id === "string" && request_id) {
      ...
      const { data: request, error: requestError } = await serviceClient
        ...
        .select("status, passenger_trip_id, fee, sub_start_date, sub_end_date, pickup_is_accepted, dropoff_is_accepted, pickup_time_is_accepted, fee_is_accepted, sub_start_is_accepted, sub_end_is_accepted")
        .eq("id", request_id)
        .maybeSingle();
      if (requestError || !request) throw new Error("Request not found.");

      // 2. Security Guard: Prevent intents on terminal requests
      const terminalStatuses = ["completed", "cancelled", "rejected"];
      if (terminalStatuses.includes(request.status)) {
        throw new Error(`Cannot create a payment intent for a ${request.status} request.`);
      }

      // 3. Security Guard: Prevent double-deposit if subscription already generated
      const { data: existingSub } = await serviceClient
        .from("tumpang_subscription")
        .select("id")
        .eq("tumpang_request_id", request_id)
        .maybeSingle();

      if (existingSub) {
        throw new Error("This request has already been paid and finalized into a subscription.");
      }

      // 4. Require all negotiations to be accepted
    ...
    // 5. Return both client_secret and the strict server-calculated amount
