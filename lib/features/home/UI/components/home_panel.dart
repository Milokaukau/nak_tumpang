          ...
          children: [
            ActiveSubscriptionCard(
              ...
              onDetailsPressed: () async {
                final requiresRefresh = await Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (_) => SubscriptionDetailScreen(
                      subscription: sub,
                      currentUserId: viewModel.currentUserId ?? '',
                      role: 'driver', // (or 'driver' depending on the view)
                    ),
                  ),
                );

                // If the screen popped with `true` (cancelled), reload the home data
                if (requiresRefresh == true) {
                  viewModel.refreshCurrentUserData();
                }
              },
              onExceptionPressed: () => viewModel.openCantFetchPanel(sub),
            ),
        ...
        return Column(
          children: [
            ActiveSubscriptionCard(
              ...
              onExceptionPressed: () => viewModel.openNoNeedFetchPanel(sub),
            ),
            if (index != viewModel.activeSubscriptions.length - 1)
              const Padding(
                padding: EdgeInsets.symmetric(vertical: 16),
